...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	// Registre d'utilisaturs/roles, pour l'authentification
	@Override
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {

	auth	 	
	 	.inMemoryAuthentication()
	 		.withUser("leila").password( encoder().encode("password")).roles("USER")
	 		.and()
	 		.withUser("didier").password(encoder().encode("password")).roles("USER", "ADMIN");
		
	}
	
	// Règles d'accès aux ressources (Politique de sécurité)
	@Override
